...
3. Proactive chitchat must have isolated session scope and explicit control commands:
   ...
   - Chitchat is enabled with `/chitchat_on` and disabled with `/chitchat_off`.
...
4. `/chitchat_on` -> confirm ON
5. `/chitchat_off` and `/next_time` -> confirm OFF or already OFF
6. `/help` -> includes Feishu-specific session/chitchat commands
...
2. If chitchat is OFF, Feishu chitchat proactive delivery is suppressed (not sent).
...
3. Dedicated proactive chitchat session + `/chitchat_on` and `/chitchat_off`: satisfied.
4. `/next_time` ambiguity: resolved by treating it as compatibility alias to explicit OFF behavior.

## 5. Operational Notes

1. Use `/sessions` to inspect per-chat Feishu session keys.
2. Use `/resume` to move between normal memory sessions.
3. Keep chitchat OFF by default; enable only when proactive IM chitchat is desired.
4. For debugging routing state, inspect `<workspace>/.feishu_sessions.json`.
